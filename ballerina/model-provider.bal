    ...
    *ai:ModelProvider;
    ...
    public isolated function init(@display {label: "API Key"} string apiKey,
        ...
        if apiType == CHAT_COMPLETIONS {
            ...
            // Responses API: create a chat:Client (same type as Chat Completions)
            http:ClientHttp1Settings?|error http1Settings = connectionConfig?.http1Settings.cloneWithType();
            if http1Settings is error {
                return error ai:Error("Failed to clone http1Settings", http1Settings);
            }
    ...
    returns ai:ChatAssistantMessage|ai:LlmError {
        do {
            ...
            if hasToolCallResponse {
                ...
                if toolCalls is chat:ChatCompletionMessageCustomToolCall[] && toolCalls.length() > 0 {
                    return error ai:LlmError("Custom tools are not supported, Found tool call: " + toolCalls[0].toJsonString());
                }
...
# Determines whether the given model supports the `temperature` sampling parameter.
#
# Reasoning models (o1, o3, o4-mini) and GPT-5 series do not accept `temperature`
# and return HTTP 400 if it is included in the request.
#
